...
# Check that R is at least version 4.4.0
if (getRversion() < "4.4.0") {
  stop("R version 4.4.0 or higher is required. Please update your R installation.")
}
# ---------------------------
...
install_and_load <- function(packages) {
  for (pkg in packages) {
    if (!requireNamespace(pkg, quietly = TRUE)) {
      if (pkg %in% c("rtracklayer", "GenomicRanges")) {
...
if (length(args) == 0) {
  stop("Error: No arguments provided. Please supply the required key-value pairs.")
}
if (length(args) %% 2 != 0) {
...
# Validate arguments
# ---------------------------
# Make genome_gtf optional by not requiring it
if (annotation_dir == "" || functional_dir == "" || output_dir == "") {
  stop("Error: --outputDir with functional (default folder: functional_annotation) and \nannotation (default: annotation/corrected_with_introns.gtf) directories inside must be provided.")
}
...
# Dynamically select columns based on what is available.
base_cols <- c("seqnames", "start", "end", "width", "strand", "source", "type", "score", "phase", "gene_id", "transcript_id")
extra_cols <- c("cmp_ref", "gene_name", "ref_gene_id", "exon_number", "cmp_ref_gene")
existing_extra <- extra_cols[extra_cols %in% colnames(stringtie_anno)]
required_cols <- c(base_cols, existing_extra)
transcripts <- as.data.frame(stringtie_anno)[, required_cols, drop = FALSE]
...
# Check if genome_gtf argument is provided
if (genome_gtf != "") {
  # genome_gtf is expected to be the full path to the GTF file
  ref_anno_path <- genome_gtf
  
  # Validate that the provided genome_gtf is a file and has a .gtf extension
  ...
  warning("Warning: No --genome_gtf provided - Skipping reference-based steps.")
}
...
#pfam2go_path <- file.path(functional_dir, "pfam2go.txt")  

if (!file.exists(orfs_path) && !file.exists(blastp_path) && !file.exists(blastx_path) && !file.exists(pfam_path)) {
  stop("Error: Functional annotation files are missing.")
}

if (!file.exists(orfs_path)) {
  warning("Warning: transcripts_with_orfs.txt is missing. ORF-related annotations will be skipped.")
}

# Function to safely load a file or return an empty data frame
safe_read <- function(path, read_fun, ...) {
  if (file.exists(path)) {
    read_fun(path, ...)
  } else {
    message(sprintf("Warning: File %s not found. Using empty data.", path))
    data.frame()
  }
}

# Load functional annotations
orfs <- safe_read(orfs_path, read.table, stringsAsFactors = FALSE)
uniprot_blastp <- safe_read(blastp_path, read.table, stringsAsFactors = FALSE)
uniprot_blastx <- safe_read(blastx_path, read.table, stringsAsFactors = FALSE)
#pfam <- rhmmer::read_domtblout(pfam_path)
pfam <- safe_read(pfam_path, read.delim, comment.char="#", header = FALSE, stringsAsFactors = FALSE)
...
# Process ORFs if available
if (nrow(orfs) > 0) {
  orfs <- orfs %>%
    tidyr::separate(V4, into = c("transcript_id", "orf_info"), sep = ":") %>%
    dplyr::select(transcript_id, orf_info)
} else {
  orfs <- data.frame(transcript_id = character(), orf_info = character(), stringsAsFactors = FALSE)
}

# Process BLASTp if available
if (nrow(uniprot_blastp) > 0) {
  ...
  uniprot_blastp <- uniprot_blastp %>%
    dplyr::mutate(transcript_id = stringr::str_replace(transcript_id, "^((?:[^.]+\\.){2}[^.]+).*", "\\1"))
} else {
  uniprot_blastp <- data.frame(transcript_id = character(), hit_id = character(), percent_identity = numeric(),
                               alignment_length = numeric(), mismatches = numeric(), gap_opens = numeric(),
                               query_start = numeric(), query_end = numeric(), subject_start = numeric(),
                               subject_end = numeric(), evalue = numeric(), bit_score = numeric(), stringsAsFactors = FALSE)
}

# Process BLASTx if available
if (nrow(uniprot_blastx) > 0) {
  colnames(uniprot_blastx) <- c("transcript_id", "hit_id", "percent_identity", "alignment_length",
                                "mismatches", "gap_opens", "query_start", "query_end",
                                "subject_start", "subject_end", "evalue", "bit_score")
} else {
  uniprot_blastx <- data.frame(transcript_id = character(), hit_id = character(), percent_identity = numeric(),
                               alignment_length = numeric(), mismatches = numeric(), gap_opens = numeric(),
                               query_start = numeric(), query_end = numeric(), subject_start = numeric(),
                               subject_end = numeric(), evalue = numeric(), bit_score = numeric(), stringsAsFactors = FALSE)
}
...
# Only process Pfam data if columns exist
if (ncol(pfam) > 0) {
  colnames(pfam) <- c("target_name", "target_accession", "tlen", 
                         "query_name", "query_accession", "qlen", 
                         "full_Evalue", "full_score", "full_bias", 
                         "domain_number", "total_domains", 
                         "c_Evalue", "i_Evalue", 
  ...
  pfam_filtered <- pfam %>%
    dplyr::mutate(
      coverage = (ali_to - ali_from + 1) / qlen * 100
    ) %>%
    filter(
      i_Evalue < 1e-5,
      domain_score > 10, # Bit score > 20: real threshold
      coverage > 50
    )
  ...
  pfam_info <- pfam_filtered %>%
    dplyr::select(
      ...
      Pfam_domain_accession = target_accession, 
  ...
  pfam_info <- data.frame()
  best_pfam <- data.frame(transcript_id = character(), best_Pfam_hit = character(), stringsAsFactors = FALSE)
  pfam_hits <- data.frame(transcript_id = character(), hit = character(), stringsAsFactors = FALSE)
}
...
# Process BLASTp hits if available
if (nrow(uniprot_blastp) > 0) {
...
# Process BLASTx hits if available
if (nrow(uniprot_blastx) > 0) {
...
# Select and process relevant columns from BLASTp and BLASTx if non-empty
if (nrow(signif_blastp) > 0) {
  signif_blastp_info <- signif_blastp %>%
    #dplyr::select(transcript_id, hit_id) %>%
    dplyr::mutate(
      UniProt_accession = str_extract(hit_id, "(?<=\\|)[A-Z0-9]+(?=\\|)"),
      UniProt_entry_name = str_extract(hit_id, "(?<=\\|)[A-Z0-9_]+$")
    )
  best_blastp <- signif_blastp_info %>%
    dplyr::group_by(transcript_id) %>%
  ...
  blastp_hits <- signif_blastp_info %>%
    dplyr::select(transcript_id, UniProt_entry_name) %>%
    dplyr::rename(hit = UniProt_entry_name)
} else {
  signif_blastp_info <- data.frame()
  best_blastp <- data.frame(transcript_id = character(), best_SwissProt_blastp_hit = character(), stringsAsFactors = FALSE)
  blastp_hits <- data.frame(transcript_id = character(), hit = character(), stringsAsFactors = FALSE)
}

if (nrow(signif_blastx) > 0) {
  ...
  best_blastx <- signif_blastx_info %>%
    dplyr::group_by(transcript_id) %>%
    dplyr::arrange(desc(percent_identity), evalue) %>%
    slice(1) %>%
    dplyr::ungroup() %>%
  ...
  blastx_hits <- signif_blastx_info %>%
    dplyr::select(transcript_id, UniProt_entry_name) %>%
    dplyr::rename(hit = UniProt_entry_name)
} else {
  signif_blastx_info <- data.frame()
  best_blastx <- data.frame(transcript_id = character(), best_SwissProt_blastx_hit = character(), stringsAsFactors = FALSE)
  blastx_hits <- data.frame(transcript_id = character(), hit = character(), stringsAsFactors = FALSE)
}
...
all_hits_combined <- dplyr::bind_rows(
  pfam_hits,
  blastx_hits,
  blastp_hits
) %>%
...
# Remove redundant transcripts with no functional information

functional_transcripts <- transcripts_with_hits[transcripts_with_hits$type=="gene" | 
                                                  (transcripts_with_hits$type!="gene" & ( !(is.na(transcripts_with_hits$best_SwissProt_blastp_hit)) | 
...
# ---------------------------
final_gtf_path <- file.path(functional_dir, "annotation_part_I.gtf")
# Convert to GRanges for rtracklayer::export 
if (!inherits(functional_transcripts_updated, "GRanges")) {
  functional_transcripts_updated <- makeGRangesFromDataFrame(
    functional_transcripts_updated,
    keep.extra.columns = TRUE,
...
#rtracklayer::export(functional_transcripts_updated, file = final_gtf_path, sep = "\t", quote = FALSE, row.names = FALSE, col.names = TRUE)
